...
void do_C_e_H_optim_compute_P_HT(const char *P_HT_fname,
    ...
    DimGrid = dim3(ceil(M / float(TILE_SIZE)), ceil(N / float(TILE_SIZE)), 1);
    DimBlock = dim3(TILE_SIZE, TILE_SIZE, 1);
    C_e_H_optim_compute_P_HT_kernel<<<DimGrid, DimBlock>>>(e_device, C_device, H_device, H_indices_device, H_indptr_device, P_HT_device, M, N, L);
    cudaDeviceSynchronize();
    finish = clock();
    printf("Latency: %fms\n", (double)(finish - start) * 1000 / CLOCKS_PER_SEC);
...
int main(int argc, char **argv) {
    ...
    do_C_e_H_optim_compute_P_HT(argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
    return 0;
}
